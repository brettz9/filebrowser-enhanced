...
exports.main = function() {
            ...
            return buttons.revealDirectory || ui.ActionButton({
                ...
                onClick: function (e) {
                    ...
                    try {
                        ...
                        if (fileBrowserResponses.isDirectory(fileName)) {
                            var dir = file.dirname(fileName);
                            // Todo: deal with all the reveal()/launch() in this file per prefs.revealLaunchNoPrompt
    ...
    var mod, hotkey, workers = {}; // Tried a WeakMap but didn't work
    function createMod () {
        if (mod) {
            mod.destroy();
        }
        var modOpts = {
            ...
            contentScriptFile: [data.url('bower_components/jawbar/src/jawbar.js'), data.url('jml.js'), data.url('directoryMod.js')], // Todo: i18n-ize directoryMod.js, supplying it strings
            contentScriptWhen: 'ready',
            contentScriptOptions: { // any JSON-serializable key/values
                folderImage: data.url('Yellow_folder_icon_open.png')
            },
            contentStyleFile: [data.url('bower_components/jawbar/src/styles/jawbar.css')],
            attachTo: [
                'top',
                'existing',
                'frame'
            ...
            onAttach: function (worker) {
                if (!fileBrowserResponses.urlIsDirectory(worker.tab.url)) {
                    return;
                }
                workers[worker.tab.url] = worker;
                worker.port.emit(
                    'getNativePathFromFileURLIfADirectory',
                    fileBrowserResponses.getNativePathFromFileURL(worker.tab.url)
                );
                relayResponse.bind(fileBrowserResponses)()(worker);
            }
        };
        if (prefs.conserveSpace) {
            modOpts.contentStyleFile.push(data.url('directoryMod.css'));
        }
    ...
    simplePrefs.on('', function (pref) {
        switch (pref) {
            ...
            case 'conserveSpace': case 'revealLaunchNoPrompt':
                createMod();
                break;
        }
    });
    ...
    function updateDirectoryState(tab) {
        ...
        if (isFileURL) {
            // var isDir = fileBrowserResponses.urlIsDirectory(tab.url);
    ...
    hotkey = require('sdk/hotkeys').Hotkey({
        combo: 'accel-shift-l',
        onPress: function () {
            // While this oddly works if we are in a different tab,
            // if in the same tab, though the "selectInput" event
            // does get activated, it doesn't actually work to focus!
            var worker = workers[tabs.activeTab.url];
            worker.tab.activate();
            worker.port.emit('selectInput');
        }
    });
    ...
    tabs.on('pageshow', function (tab) {
        ...
        // var worker = workers[tab.url];
        // worker.port.emit('getNativePathFromFileURLResponse', tab.url);
        updateDirectoryState(tab);
    });
};
