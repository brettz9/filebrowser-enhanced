...
console.dir(history);
            var optValues = bookmarks.concat(history).sort(function (item) {
                return item.date;
            }).map(function (item) {
                return getNativePathFromFileURL(item.url);
            });
            cb(optValues);
        });
    });
...
exports.autocompleteValues = function (data, emit, on, worker, sendResponse) {
    ...
    if (searchTermsOnly) {
        ...
        searchBookmarksAndHistory(searchTerms, function (optValues) {l('a');
            sendResponse({
                listID: data.listID,
                optValues: optValues,
